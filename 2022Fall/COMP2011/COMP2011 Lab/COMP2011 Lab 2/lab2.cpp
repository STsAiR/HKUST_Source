...
int main()
{
    ...
    */
    ...
    for (int i = numOfDigits ; i > 0 ; i--){
        int power1 = static_cast<double> (pow(10,i));
        int subStr = inputNumber % power1;
        for(int j = 0 ; j < i; j++){
            int power2 = static_cast<double> (pow(10,j));
            int newSubStr = 0;
            newSubStr = subStr / power2;
            if (inputNumber % newSubStr == 0){
                totalCount++;
                cout << newSubStr << endl;
            }
        }
